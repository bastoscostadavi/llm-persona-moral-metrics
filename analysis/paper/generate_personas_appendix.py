...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        ...
        default=None,
        help=(
            "Optional persona_id_remap.csv. When given, personas are listed in "
            "new-id order (ascending shift from baseline) and numbered by new id, "
            "overriding --personas/--start/--max."
        ),
    )
    return p.parse_args()
...
def _write_remapped(remap_path: Path, output: Path) -> None:
    """Emit personas in new-id order (ascending shift), numbered by new id."""
    rows = list(csv.DictReader(remap_path.open(encoding="utf-8")))
    rows.sort(key=lambda r: int(r["new_id"]))
    lines = ["% Auto-generated by analysis/paper/generate_personas_appendix.py --remap"]
    lines.append("% Personas numbered by ascending average distance from the no-persona baseline.")
    lines.append("\\begin{enumerate}")
    lines.append("\\setcounter{enumi}{-1}")
    for row in rows:
        text = latex_escape(str(row["persona_text"]).strip())
...
def main() -> None:
    ...
    if args.remap is not None:
        _write_remapped(args.remap, args.output)
        return
